        ...
        while (continuing) { // 进入循环，只要标志变量为真就继续执行
            switch (getStack(address)) { // 根据缓冲区中指定地址的值进行不同的操作
                case "read": // 如果缓冲区中指定地址的值为 "read"
                    ...
                    let  temp_read = stack.pop()
                    console.log("read_fd:",r_fd,",parseInt(read_addr):",parseInt(read_addr),",parseInt(stack.pop()):",parseInt(temp_read))
                    read(r_fd, parseInt(read_addr), parseInt(temp_read)) // 调用读取函数，将读取文件描述符、读取地址和读取字节数作为参数传递
                    break;
                case "write": // 如果缓冲区中指定地址的值为 "write"
                    ...
                    let  temp_write = stack.pop()
                    console.log("write_fd:",w_fd,",parseInt(write_addr):",parseInt(write_addr),",parseInt(stack.pop()):",parseInt(temp_write))
                    write(w_fd, parseInt(write_addr), parseInt(temp_write)) // 调用写入函数，将写入文件描述符、写入地址和写入字节数作为参数传递
                    break;
